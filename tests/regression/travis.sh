...
for repo in clojure d go node java python php ruby rust scala ; do
    ...
    cd "$repo"
    output="$(oms run --silent message -a name="Peter" | jq -c .)"
...
echo "===== Testing base64 ====="
cd base64
output="$(oms run --silent encode -a content="Peter")"
if [ "$output" != 'UGV0ZXI=' ] ; then echo "$output"; exit 1; fi
output="$(oms run --silent decode -a content="UGV0ZXI=")"
if [ "$output" != 'Peter' ] ; then echo "$output"; exit 1; fi

echo "===== Testing hashes ====="
cd ../hashes
output="$(oms run --silent digest -a method="sha1" -a data="foo" | jq -c .)"
if [ "$output" != '{"method":"sha1","digest":"0BEEC7B5EA3F0FDBC95D0DD47F3C5BC275DA8A33"}' ] ; then echo "$output"; exit 1; fi
output="$(oms run --silent hmac -a method="sha1" -a data="hello world" -a secret="my secret" | jq -c .)"
if [ "$output" != '{"method":"sha1","digest":"9F60EE4B05E590A7F3FAC552BFB9D98FA46F78D9"}' ] ; then echo "$output"; exit 1; fi

# echo "===== Testing jwt ====="
# cd ../jwt
# output="$(oms run --silent sign -a data="hello world" -a secret="abc" -a expiresIn="2h" | jq -c .)"
...
# output="$(oms run --silent verify -a token="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjoiaGVsbG8gd29ybGQiLCJpYXQiOjE1NDM5MjMyMTAsImV4cCI6MTU0MzkzMDQxMH0.FCsstg1m01goffz0cFYxZIUe0uPybUAqzGRnZPJgGBw" | jq -c .)"
